...
if __name__ == "__main__":
    ...
    with Pool(N_WORKERS) as pool:
        partial_counts = pool.map(process_file, filepaths)
    ...
    json.dump(dict(global_counts), open("results/counts_multiprocessing.json", "w"))
    ...
    with open(RESULTS_FILE, "a", newline="") as f:
        ...
        if not file_exists:
            writer.writeheader()
        writer.writerow({
            "version": f"multiprocessing_{N_WORKERS}",
            "read": "N/A",
            "tokenize": "N/A",
            "count": "N/A",
